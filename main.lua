...
local function active_pane()
    if not dp or not dp.tabs then return 1 end
...
local function apply_dual_tab_patch()
    ...
    Tab.build = function(self, ...)
        if saved.tab_build then
            saved.tab_build(self, ...)
        end

        local c = self._chunks
        local tab1 = dp.tabs and cx.tabs[dp.tabs[1]]
        local tab2 = dp.tabs and cx.tabs[dp.tabs[2]]
        ...
        self._children = {
            ...
            Rails:new(c, self._tab), -- draw a rail between panes
...
local function apply_header_patch()
    Header.cwd = function(self)
        ...
        local tab1 = dp.tabs and cx.tabs[dp.tabs[1]]
        local tab2 = dp.tabs and dp.tabs[2] and cx.tabs[dp.tabs[2]]
...
local function restore_all()
    ...
    if saved.tabs_height then Tabs.height = saved.tabs_height end
...
local function activate()
    ...
    ps.sub("ind-watch", function(args)
        if not dp or not dp.tabs then return args end
        local t1 = cx.tabs[dp.tabs[1]]
        local t2 = cx.tabs[dp.tabs[2]]
        if not t1 or not t2 then return args end

        args.files = { t1.current.file, t2.current.file }
...
local function parse_act(job)
    ...
    if type(job) == "table" then
        if job.args then
            if type(job.args) == "table" then
                if job.args[1] then
                    return tostring(job.args[1])
                elseif type(job.args.args) == "string" then
                    return job.args.args:match("%S+")
                end
            elseif type(job.args) == "string" then
                return job.args:match("%S+")
            end
        end
        if job[1] then
            return tostring(job[1])
        end
    end
    return nil
end
...
local function entry(st, job)
    local act = parse_act(job)

    if act == "spl_toggle" or act == "toggle" then
        spl_toggle()
    elseif act == "spl_switch_tab" or act == "next_pane" then
        spl_switch_tab()
    elseif act == "spl_copy" or act == "copy_files" then
        spl_copy()
    elseif act == "spl_move" or act == "move_files" then
        spl_move()
    elseif act == "spl_preview" or act == "preview" or act == "toggle_zoom" then
        spl_preview()
    elseif act == "spl_activate" or act == "activate" then
        activate()
    elseif act == "spl_deactivate" or act == "deactivate" then
        deactivate()
    end
end
